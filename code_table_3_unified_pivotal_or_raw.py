...
#   STAT_MODE = "pivotal" -> fit on Alessio's pivotalized statistic
#
# IMPORTANT:
...
#
# This keeps the model architecture unchanged while allowing
# the pivotalized implementation.
# ============================================================
...
#   "pivotal" -> Alessio-style pivotalized statistic:
#                sqrt(T_n * (1-rho_n)/(1+rho_n)) * SR_hat_n
STAT_MODE = "pivotal"   # change only this line
...
def pivotalize_sharpe(sr: float, T: int, rho: float) -> float:
    """
    Alessio-style pivotalized Sharpe statistic:
...
def build_cross_section(csv_path: str):
    ...
    for name in ret_df.columns:
        ...
        if STAT_MODE == "raw":
            ...
            # Alessio-style implementation:
            # Transform the observed statistic, but keep sigma_0 estimated.
...
def get_bounds():
    ...
    """
    # In pivotal mode, the transformed statistic may have larger scale than raw SR,
    # so allow a wider upper bound for sigma_0 and sigma_1.
    sigma_upper = 2.0 if STAT_MODE == "raw" else 20.0

    return [
        (-10.0, 10.0),                             # a -> pi0
        (np.log(1e-10), np.log(20.0)),            # b -> delta1 >= 0
...
def initial_points():
    ...
    if STAT_MODE == "raw":
        delta_grid = [1e-6, 0.01, 0.03, 0.05, 0.10, 0.20]
        sigma0_grid = [0.02, 0.05, 0.10, 0.20]
        extra_grid = [0.001, 0.01, 0.03, 0.05, 0.10]
    else:
        # In pivotal mode, allow broader scales and shifts.
        delta_grid = [1e-6, 0.01, 0.03, 0.05, 0.10, 0.20, 0.50, 1.0, 2.0]
...
def compute_table_row(K, x, c_vec):
    ...
    # In RAW mode, c_vec varies across predictors.
    # In PIVOTAL mode, c_vec is constant at z_(1-alpha/2).
...
def main():
    ...
    print(f"\nTable 2 ({STAT_MODE} mode):")
    print(table2_display.to_string(index=False))

    suffix = "raw" if STAT_MODE == "raw" else "pivotal"
    table2_name = f"Table2_unified_{suffix}.csv"
    stats_name = f"Section6_predictor_stats_{suffix}.csv"

    table2_display.to_csv(table2_name, index=False)
